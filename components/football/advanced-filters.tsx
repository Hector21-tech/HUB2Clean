...
import { Filter, X, Calendar, Target } from 'lucide-react'
...
export function AdvancedFilters({
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-gradient-to-br from-background via-background/95 to-background rounded-xl border border-border shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
            ...
            <div>
              ...
              <div className="space-y-2">
                {STATUS_OPTIONS.map((option) => (
                  <label key={option.value} className="flex items-center gap-3 cursor-pointer p-2 hover:bg-accent/50 rounded-lg">
                    <input
                      type="checkbox"
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {POSITION_OPTIONS.map((option) => (
                  <label key={option.value} className="flex items-center gap-2 cursor-pointer p-2 hover:bg-accent/50 rounded-lg">
                    <input
                      type="checkbox"
                      ...
                      className="w-4 h-4 rounded border-border bg-background text-primary focus:ring-primary"
                    />
                    <span className="text-foreground/80 text-sm">{option.label}</span>
                  </label>
                ))}
              </div>
            </div>
                ...
                {WINDOW_STATUS_OPTIONS.map((option) => (
                  <label key={option.value} className="flex items-center gap-3 cursor-pointer p-2 hover:bg-accent/50 rounded-lg">
                    ...
                    <div className="flex-1">
                      <div className="text-foreground/80 text-sm">{option.label}</div>
                      <div className="text-foreground/50 text-xs">{option.description}</div>
                    </div>
                  </label>
                ))}
              </div>
